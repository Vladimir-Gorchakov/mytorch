...
template <typename T> Tensor<T>::Tensor(const shape_t &shape) : shape_(shape) {
  if (!shape_.empty()) {
    size_t total_len = 1;
...
template <typename T> size_t Tensor<T>::numel() const {
  size_t n = 1;
  for (size_t s : shape_) {
    n *= s;
  }
...
template <typename T> T &Tensor<T>::operator[](const shape_t &index) {
...
template <typename T> Tensor<T> Tensor<T>::operator*(const T &other) const {
  Tensor<T> output{};
