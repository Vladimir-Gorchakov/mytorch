...
class Operand {
  public:
    ...
    Operand(T* data_ptr, shape_t strides) : data_ptr_(data_ptr), strides_(strides) {}
};
...
class TensorIterator {
  ...
  public:
    TensorIterator(const std::vector<Tensor<T>*>& inputs, const std::vector<Tensor<T>*>& outputs);
...
shape_t TensorIterator<T>::broadcast_shape(const std::vector<Tensor<T>*>& inputs) {
    ...
    for (size_t i = 1; i <= shape_size; ++i) {
        for (auto tensor : inputs) {
            ...
            if (shape[s_i] != 1 && tensor->shape()[t_i] != 1 && shape[s_i] != tensor->shape()[t_i])
                [[unlikely]] {
                throw exception::shape_exception{
...
template <typename T>
void TensorIterator<T>::_move_ptrs(std::vector<T*>& ptrs, std::vector<shape_t*>& strides, int step,
                                   size_t dim) {
