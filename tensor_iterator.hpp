...
public:
  ...
  Operand(T *data_ptr, shape_t strides)
      : data_ptr_(data_ptr), strides_(strides) {}
};
...
public:
  TensorIterator(const std::vector<Tensor<T> *> &inputs,
...
template <typename T>
TensorIterator<T>::TensorIterator(const std::vector<Tensor<T> *> &inputs,
                                  const std::vector<Tensor<T> *> &outputs) {
  ...
  for (auto tensor : outputs) {
    if (!(shape_ == tensor->shape())) {
      throw exception::shape_exception{std::format(
          "Shape exception: output tensor shape must be {} but got {}",
...
template <typename T> size_t TensorIterator<T>::numel() const {
...
template <typename T>
shape_t
TensorIterator<T>::broadcast_shape(const std::vector<Tensor<T> *> &inputs) {
  size_t shape_size = 0;
  for (auto tensor : inputs) {
...
shape_t TensorIterator<T>::compute_strides(shape_t strides,
  ...
  for (size_t i = 0; i < shape_.size(); ++i) {
    if (i <
        shape_.size() -
            strides.size()) { // Гарантированно strides.size() <= shape_.size()
      continue;
    }
...
template <typename T> shape_t TensorIterator<T>::shape() { return shape_; }
...
public:
  ...
  TensorIterator<T> build() { return TensorIterator<T>{&inputs_, &outputs_}; }
};
